...
_=>generate_random_str_of_rn_len
    ...
    loop {
        ...
        let mut file = OpenOptions::new().write(true).truncate(true).open(format!("{dir_name}/{FILE_NAME}")).expect("can not open file");
        file.write_all(&random_chars[..]).expect("file can not be written");
        
        let status = Command::new(executable.clone()).arg(format!("{dir_name}/{FILE_NAME}")).status().expect("ERROR:failed to execute the program");
        if let Some(code)=status.code() {
            if code==99 {
               let mut failure_case_file = File::create(format!("{dir_name}/internal_error_99_{num_of_cycles}.zig")).unwrap();//if I fail here so be it.
            ...
            let mut failure_case_file = File::create(format!("{dir_name}/signal_failure_{num_of_cycles}.zig")).unwrap();
            failure_case_file.write_all(&random_chars[..]).unwrap();
        }
